        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN || event.getAction() == MotionEvent.ACTION_MOVE) {
            if (pointX > btn_sound_x && pointX < btn_sound_x + bmpSound.getWidth()) {
                if (pointY > btn_sound_y && pointY < btn_sound_y + bmpSound.getHeight()) {
                    ...
                    //改变当前游戏状态为开始游戏
                    if(MySurfaceView1.soundFlag){
                        MySurfaceView1.soundFlag = false;
                        if(MySurfaceView.gameState == MySurfaceView1.GAME_MENU)
                            MySurfaceView1.mediaPlayer2.start();
                        else if(MySurfaceView.gameState == MySurfaceView1.GAME_PAUSE)
                            MySurfaceView1.mediaPlayer2.start();
                    } else {
                        MySurfaceView1.soundFlag = true;
                        MySurfaceView1.mediaPlayer.pause();
                        MySurfaceView1.mediaPlayer2.pause();
                    }
                }
            }
        }
